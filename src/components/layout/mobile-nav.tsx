...
export function MobileNav({ isLoggedIn, userName }: MobileNavProps) {
  ...
  return (
    <>
      ...
      {/* Mobile Menu */}
      {open && (
        <div className="md:hidden absolute top-full left-0 right-0 bg-green-50 border-t border-gray-200 shadow-xl z-50">
          <nav className="container-page py-4 flex flex-col gap-1">
            {NAV_LINKS.map((link) => (
              <Link
                ...
                className="px-4 py-3 rounded-xl text-gray-700 hover:text-accent hover:bg-white/60 transition-colors font-medium text-sm"
              >
                {link.label}
              </Link>
            ))}
              ...
              {isLoggedIn ? (
                <>
                  ...
                  <form action="/auth/signout" method="post">
                    <button
                      type="submit"
                      className="px-4 py-3 rounded-xl text-gray-700 hover:text-accent hover:bg-white/60 transition-colors font-medium text-sm w-full text-left"
                    >
                      Sign out
                    </button>
                  </form>
