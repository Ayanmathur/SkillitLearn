      ...
      <header className="sticky top-0 z-50 bg-[#1a1a2e] shadow-lg">
        <div className="container-page flex items-center justify-between h-14">
          ...
          <nav className="flex items-center gap-2 text-xs text-white/60 flex-wrap">
            <Link href="/admin/careers" className="hover:text-white">Careers</Link><span>/</span>
            <Link href={`/admin/careers/${skill.path.career.id}`} className="hover:text-white">{skill.path.career.name}</Link><span>/</span>
            <Link href={`/admin/paths/${skill.path.id}`} className="hover:text-white">{skill.path.name}</Link><span>/</span>
